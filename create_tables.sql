...
CREATE TABLE IF NOT EXISTS Books (
    ...
    FOREIGN KEY (genre_id) REFERENCES Genres(genre_id) ON DELETE
    SET
        NULL
);
...
CREATE TABLE IF NOT EXISTS Loans (
    ...
    loan_period_days INTEGER NOT NULL CHECK (
        loan_period_days BETWEEN 1
        AND 30
    ),
    loan_date TEXT NOT NULL DEFAULT (date('now')),
    due_date TEXT NOT NULL GENERATED ALWAYS AS (
        date(loan_date, '+' || loan_period_days || ' days')
    ) VIRTUAL,
    return_date TEXT,
